import { CacheType, Client, Collection, CommandInteractionOptionResolver, Interaction, TextChannel, Routes, REST } from "discord.js";
import EventEmitter from "events";
import { Mode } from "..";
import { Command } from "../Types/Command";

export class LFCommand extends EventEmitter {
    ...
    private handleCommand(){
        const listener = async (interaction:Interaction<CacheType>) => {
            if (interaction.isChatInputCommand()) {
                ...
                if (!command)
                    return this.commands.delete(interaction.commandName)
    
                if (command.nsfw && !(<TextChannel>interaction.channel)?.nsfw) {
                    return this.emit('isNotNSFWChannel');
                }
        ...
        this.client.on('interactionCreate', async (interaction) => { listener(interaction) });
    }
    ...
    public async registerCommands(
		...
		try {
			if (mode === 'prod' || mode === 1) {
                ...
                this.emit('onGuildCmdReload', this.commands.map((command) => command.data).length);
